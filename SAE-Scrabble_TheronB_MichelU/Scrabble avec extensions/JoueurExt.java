...
public class Joueur {
	...
	* résultat : vrai ssi ce coup est valide, céest-é-dire accepté par
	* CapeloDico et satisfaisant les régles détaillées plus haut
	*/
	public boolean joueMot(Plateau p, MEE s, int[] nbPointsJet) {
		...
		int nbLig=ligneCharToInt(ligne);
		while (nbLig>14) {
			Ut.afficher("Saisir une ligne valide entre A et O :");
			ligne = Ut.saisirCaractere();
			nbLig=ligneCharToInt(ligne);
		}
		
		Ut.afficher("Saisir la colonne :");
		int nbCol= Ut.saisirEntier();
		while (nbCol>14) {
			Ut.afficher("Saisir une colonne valide entre 0 et 14 :");
	...
	public static int ligneCharToInt(char nbLig) {
		int i=0;
	...
	/** pré-requis : cf. joueMot et le placement de mot é partir de la case
	* (numLig, numCol) dans le sens donné par sens est valide
	* action : simule le placement déun mot de this
	*/
	...
	/**
	* prÃ©-requis : les éléments de sac sont inférieurs é 26
	* action : simule lééchange de jetons de ce joueur
	*/
	public void echangeJetons(MEE sac) {

		Ut.afficher("Saisir les jetons Ã  Ã©changer : \n");
	...
	/** résultat : vrai ssi les caractéres de mot correspondent tous é des
	* lettres majuscules et léensemble de ces caractéres est un
